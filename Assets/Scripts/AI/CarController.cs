...
namespace Assets.Scripts.AI
{
    ...
    public class CarController : MonoBehaviour
    {
        ...
        // ── Отсоединённые колёса ──────────────────────────────────────────────
        private readonly HashSet<Transform> detachedWheelMeshes = new HashSet<Transform>();

        public void NotifyWheelDetached(WheelCollider wheelCollider, Transform wheelMesh)
        {
            Debug.Log($"[CarController] NotifyWheelDetached: mesh={wheelMesh?.name}, collider={wheelCollider?.name}");
            ...
            if (wheelMesh != null)
                detachedWheelMeshes.Add(wheelMesh);
        }
        ...
        private void UpdateWheelMesh(WheelCollider col, Transform mesh)
        {
            ...
            // ── Пропускаем отцепленные колёса ─────────────────────────────────
            if (detachedWheelMeshes.Contains(mesh))
                return;
